...
from r2r.vision_transformer import vit_base_patch16_224
...
class EnvBatch(object):
    ...
    def process_features(self, images, house_id, navigable_idx, viewpointId, instr_id):
        ...
        if is_pano:
            with torch.no_grad():
                feats, flops, params = profile(self.vision_backbone, inputs=(images, house_id, navigable_idx, viewpointId, instr_id), verbose=False)
                gflops = flops / (10**9)
                txt = f"(ViT) is_pano Gflops: {gflops}\n"
            ...
            feats, flops, params = profile(self.vision_backbone, inputs=(images, house_id, navigable_idx, viewpointId, instr_id), verbose=False)
            gflops = flops / (10**9)
            txt = f"(ViT) not pano Gflops: {gflops}\n"
            with open(os.path.join(self.args.log_dir, 'gflops_per_traj_log.txt'), 'a') as file:
                file.write(txt)
            layer_count.total_gflops += gflops
...
class ReverieObjectNavBatch(object):
    ...
    def __init__(
        self, view_db, obj_db, instr_data, connectivity_dir, img_db_file, obj2vps, args,
        ...
        self.env = EnvBatch(connectivity_dir, img_db_file, feat_db=view_db, batch_size=batch_size, cache=cache, args=args)
        self.obj_db = obj_db
