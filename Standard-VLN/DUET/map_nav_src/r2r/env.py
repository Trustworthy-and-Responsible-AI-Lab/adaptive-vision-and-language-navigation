...
class EnvBatch(object):
    ...
    def __init__(self, connectivity_dir, img_db_file, args, scan_data_dir=None, feat_db=None, batch_size=100, cache=False):
        """
        ...
        self.vision_backbone = vit_base_patch16_224(
            ...
            drop_path_rate=0.1,
            mode=self.args.mode,
            vit_path=self.args.vit_path,
            k_ext=self.args.k_ext,
            ee_decay=self.args.ee_decay,
            lsh_threshold=self.args.lsh_threshold
        ).to(torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")).eval()
    ...
    def process_features(self, images, house_id, navigable_idx, viewpointId, instr_id):
        ...
        if is_pano:
            with torch.no_grad():
                feats, flops, params = profile(model, inputs=(images, house_id, navigable_idx, viewpointId, instr_id), verbose=False)
                gflops = flops / (10**9)
            ...
            feats, flops, params = profile(model, inputs=(images, house_id, navigable_idx, viewpointId, instr_id, self.mode), verbose=False)
            gflops = flops / (10**9)
    ...
    def getStates(self, item):
        ...
        for i, sim in enumerate(self.sims):
            ...
            if self.cache:
                ...
                feature = self.process_features(images, state.scanId, navigable_idx, state.location.viewpointId, instr_id)
...
class R2RNavBatch(object):
    ...
    def __init__(
        self, view_db, instr_data, connectivity_dir, img_db_file, args,
        batch_size=64, angle_feat_size=4, seed=0, name=None, sel_data_idxs=None, cache=False,
    ):
        self.env = EnvBatch(connectivity_dir, img_db_file, args, feat_db=view_db, batch_size=batch_size, cache=cache)
