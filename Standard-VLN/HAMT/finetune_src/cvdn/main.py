...
def build_dataset(args, rank=0, is_test=False):
    ...
    train_env = dataset_class(
        feat_db, train_instr_data, args.connectivity_dir, img_db_file, args,
        batch_size=args.batch_size, 
        angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
        sel_data_idxs=None, name='train', use_player_path=args.use_player_path, cache=args.cache
    )
    if args.aug is not None:
        ...
        aug_env = dataset_class(
            feat_db, aug_instr_data, args.t, img_db_file, args, batch_size=args.batch_size,
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None, name='aug', use_player_path=args.use_player_path, cache=args.cache
        )
    else:
        aug_env = None
    ...
    for split in val_env_names:
        ...
        val_env = dataset_class(
            feat_db, val_instr_data, args.connectivity_dir, img_db_file, args, batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None if args.world_size < 2 else (rank, args.world_size), name=split, cache=args.cache
        )
...
def valid(args, train_env, val_envs, rank=-1):
    ...
    txt = f"average GFLOPs: {sum(layer_count.gflops)/len(layer_count.gflops)}\n"
    with open(os.path.join(args.log_dir, 'average_gflops.txt'), 'a') as file:
        file.write(txt)
