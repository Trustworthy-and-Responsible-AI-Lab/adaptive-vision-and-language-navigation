...
def parse_args():
    ...
    # add lmdb path
    parser.add_argument('--img_db_file', type=str, help='Path to the LMDB file')
    parser.add_argument('--vit_path', type=str, help='Path to the file that contains the ViT model checkpoint')
    
    parser.add_argument('--cache', default=False, action='store_true', help='cache image features or compute on the fly')
    
    parser.add_argument('--mode', type=str, choices=['baseline', 'efficient', 'k_only', 'k_lsh', 'k_ee', 'mue', 'lsh_only', 'ee_only', 'ee_lsh'])
    parser.add_argument('--k_ext', type=int, default=4, help='number of neighbors for k-extensions')
    parser.add_argument('--ee_decay', type=float, default=9e-4, help='decay factor for threshold for early-exiting nearby views')
    parser.add_argument('--lsh_threshold', type=float, default=0.85, help='similarity threshold for LSH')
    ...
    parser.add_argument('--max_eval', type=int, default=2349)
