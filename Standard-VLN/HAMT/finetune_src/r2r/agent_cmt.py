...
import logging
logging.getLogger("thop.profile").setLevel(logging.WARNING)
...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def extend_nav_idx(self, nav_idx):
        extended_idx = set()
        for idx in nav_idx:
            for offset in range(-4, 5):
                new_idx = idx + offset
                if 0 <= new_idx < 36:
                    extended_idx.add(new_idx)
        return sorted(extended_idx)
    ...
    def _cand_pano_feature_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            for j, cc in enumerate(ob['candidate']):
                ...
                mask_index.append(cc['pointId'])
                navigable_idx.add(cc['pointId'])
            # add [STOP] feature
            extended_idx = self.extend_nav_idx(navigable_idx)
            ...
            mask_index.append('Stop')
            # add pano context

            for idx, i in enumerate(~cand_pointids):
                if i == True:
            ...
            # create mask that ignores zero vector
            for i in mask_index:
                if i in extended_idx or i == 'Stop':
                    mask_ignore_0.append(True)
                else:
                    mask_ignore_0.append(False)
        ...
        mask_ignore_0 = torch.tensor(mask_ignore_0, dtype=torch.bool)
        mask_ignore_0 = mask_ignore_0.unsqueeze(0)
        mask_ignore_0 = mask_ignore_0.cuda()
        ...
        return ob_img_fts, ob_ang_fts, ob_nav_types, ob_lens, ob_cand_lens, mask_ignore_0
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        :return:
        """
        # get the total gflops for all instruction for batch > 1
        ...
        # need to modify profile
        # txt_embeds = self.vln_bert(**language_inputs)
        txt_embeds, lan_flops, params = profile(self.vln_bert, inputs=language_inputs, verbose=False)
        ...
        for t in range(self.args.max_action_len):
            if self.args.ob_type == 'pano':
                ob_img_feats, ob_ang_feats, ob_nav_types, ob_lens, ob_cand_lens, mask_ignore_0 = self._cand_pano_feature_variable(obs)
            ...
            visual_inputs = {
                ...
                'ob_masks': ob_masks,
                'mask_ignore_0': mask_ignore_0,
                'return_states': True if self.feedback == 'sample' else False
            }

            # t_outputs = self.vln_bert(**visual_inputs)
            t_outputs, vis_flops, params = profile(self.vln_bert, inputs=visual_inputs, verbose=False)
            vis_gflops = vis_flops / (10**9)
            txt = f"(visual)Gflops: {vis_gflops}\n"
            with open(os.path.join(self.args.log_dir, 'gflops_per_traj_log.txt'), 'a') as file:
                file.write(txt)
            layer_count.total_gflops += vis_gflops
            ...
            if train_rl or ((not np.logical_or(ended, (cpu_a_t == -1)).all()) and (t != self.args.max_action_len-1)):
                ...
                t_hist_embeds, hist_flops, params = profile(self.vln_bert, inputs=t_hist_inputs, verbose=False)
                hist_gflops = hist_flops / (10**9)
                txt = f"(history)Gflops: {hist_gflops}\n"
                with open(os.path.join(self.args.log_dir, 'gflops_per_traj_log.txt'), 'a') as file:
                    file.write(txt)
                layer_count.total_gflops += hist_gflops
    ...
    def test(self, use_dropout=False, feedback='argmax', allow_cheat=False, iters=None, max_processed=1000000):
        ...
        if use_dropout:
            ...
            self.vln_bert.eval()
            self.critic.eval()
        super().test(iters=iters, max_processed=max_processed)
    ...
    def load(self, path):
        ...
        def recover_state(name, model, optimizer):
            ...
            thop_keys = ["total_ops", "total_params"]
            state_dict = {k: v for k, v in state_dict.items() if not any(sub in k for sub in thop_keys)}
