...
def build_dataset(args, rank=0, is_test=False):
    ...
    train_env = dataset_class(
        feat_db, train_instr_data, args.connectivity_dir, img_db_file, batch_size=args.batch_size, 
    ...
    if args.aug is not None:
        ...
        aug_env = dataset_class(
            feat_db, aug_instr_data, args.connectivity_dir, img_db_file, batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
    ...
    for split in val_env_names:
        ...
        val_env = dataset_class(
            feat_db, val_instr_data, args.connectivity_dir, img_db_file, batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None if args.world_size < 2 else (rank, args.world_size), name=split, cache=args.cache, args=args
        )
...
def valid(args, train_env, val_envs, rank=-1):
    ...
    for env_name, env in val_envs.items():
        ...
        agent.test(use_dropout=False, feedback='argmax', iters=iters, max_processed=args.max_eval)
