...
class R2RBatch(object):
    ...
    def __init__(
        self, feat_db, instr_data, connectivity_dir, img_db_file, args,
        batch_size=64, angle_feat_size=4,
        seed=0, name=None, sel_data_idxs=None, cache=False,
    ):
        self.env = EnvBatch(connectivity_dir, img_db_file, feat_db=feat_db, args=args, batch_size=batch_size, cache=cache)
...
class R2RBackBatch(R2RBatch):
    def __init__(
        ...
        super().__init__(
            feat_db, instr_data, connectivity_dir, img_db_file, batch_size=batch_size, args=args,
            angle_feat_size=angle_feat_size, seed=seed, name=name, sel_data_idxs=sel_data_idxs, cache=cache
        )
